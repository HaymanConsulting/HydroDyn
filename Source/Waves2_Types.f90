...
! =========  Waves2_ParameterType  =======
  TYPE, PUBLIC :: Waves2_ParameterType
    ...
    INTEGER(IntKi)  :: NStepWave2      ! NStepWave / 2 [-]
    REAL(SiKi) , DIMENSION(:), ALLOCATABLE  :: WaveTime      ! Simulation times at which the instantaneous second order loads associated with the incident waves are determined [sec]
    REAL(SiKi) , DIMENSION(:,:), ALLOCATABLE  :: WaveElevOut2      ! Instantaneous elevation time-series of incident waves at each of the NWaveElev points where the incident wave elevations can be output [(meters)]
    REAL(SiKi) , DIMENSION(:,:), ALLOCATABLE  :: WaveElev2St      ! Instantaneous elevation time-series of incident waves at each of the  points where wave kinematics are computed [(meters)]
...
! 
   ...
   ErrMsg  = ""
    ...
    DstParamData%NWaveElevOut = SrcParamData%NWaveElevOut
...
ENDIF
IF (ALLOCATED(SrcParamData%WaveElevOut2)) THEN
  i1_l = LBOUND(SrcParamData%WaveElevOut2,1)
  i1_u = UBOUND(SrcParamData%WaveElevOut2,1)
  i2_l = LBOUND(SrcParamData%WaveElevOut2,2)
  i2_u = UBOUND(SrcParamData%WaveElevOut2,2)
  IF (.NOT. ALLOCATED(DstParamData%WaveElevOut2)) THEN 
    ALLOCATE(DstParamData%WaveElevOut2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%WaveElevOut2.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%WaveElevOut2 = SrcParamData%WaveElevOut2
ENDIF
IF (ALLOCATED(SrcParamData%WaveElev2St)) THEN
  i1_l = LBOUND(SrcParamData%WaveElev2St,1)
  i1_u = UBOUND(SrcParamData%WaveElev2St,1)
  i2_l = LBOUND(SrcParamData%WaveElev2St,2)
  i2_u = UBOUND(SrcParamData%WaveElev2St,2)
  IF (.NOT. ALLOCATED(DstParamData%WaveElev2St)) THEN 
    ALLOCATE(DstParamData%WaveElev2St(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%WaveElev2St.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%WaveElev2St = SrcParamData%WaveElev2St
ENDIF
...
ENDIF
IF (ALLOCATED(ParamData%WaveElevOut2)) THEN
  DEALLOCATE(ParamData%WaveElevOut2)
ENDIF
IF (ALLOCATED(ParamData%WaveElev2St)) THEN
  DEALLOCATE(ParamData%WaveElev2St)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  IF ( ALLOCATED(InData%WaveTime) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! WaveTime upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%WaveTime)  ! WaveTime
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! WaveElevOut2 allocated yes/no
  IF ( ALLOCATED(InData%WaveElevOut2) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! WaveElevOut2 upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%WaveElevOut2)  ! WaveElevOut2
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! WaveElev2St allocated yes/no
  IF ( ALLOCATED(InData%WaveElev2St) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! WaveElev2St upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%WaveElev2St)  ! WaveElev2St
  END IF
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      Re_Xferred   = Re_Xferred   + SIZE(InData%WaveTime)
  END IF
  IF ( .NOT. ALLOCATED(InData%WaveElevOut2) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevOut2,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevOut2,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevOut2,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevOut2,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%WaveElevOut2)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%WaveElevOut2))-1 ) = PACK(InData%WaveElevOut2,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%WaveElevOut2)
  END IF
  IF ( .NOT. ALLOCATED(InData%WaveElev2St) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElev2St,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElev2St,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElev2St,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElev2St,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%WaveElev2St)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%WaveElev2St))-1 ) = PACK(InData%WaveElev2St,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%WaveElev2St)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! WaveElevOut2 not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%WaveElevOut2)) DEALLOCATE(OutData%WaveElevOut2)
    ALLOCATE(OutData%WaveElevOut2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%WaveElevOut2.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%WaveElevOut2)>0) OutData%WaveElevOut2 = REAL( UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%WaveElevOut2))-1 ), mask2, 0.0_ReKi ), SiKi)
      Re_Xferred   = Re_Xferred   + SIZE(OutData%WaveElevOut2)
    DEALLOCATE(mask2)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! WaveElev2St not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%WaveElev2St)) DEALLOCATE(OutData%WaveElev2St)
    ALLOCATE(OutData%WaveElev2St(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%WaveElev2St.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%WaveElev2St)>0) OutData%WaveElev2St = REAL( UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%WaveElev2St))-1 ), mask2, 0.0_ReKi ), SiKi)
      Re_Xferred   = Re_Xferred   + SIZE(OutData%WaveElev2St)
    DEALLOCATE(mask2)
  END IF
