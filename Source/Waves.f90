   ...
   IF ( NWaveKinGtr0 > 0 ) THEN
      ...
      !   points where are Morison loads will be calculated.
         ...
         DO J = 1,NWaveKinGtr0 ! Loop through all points where the incident wave kinematics will be computed without stretching
            ...
            WaveDynPC0 (I,J)     = InitOut%RhoXg*tmpComplex*WaveElevxiGtr0 * COSHNumOvrCOSHDen ( WaveNmbr, InitInp%WtrDpth, 0.0_ReKi )       
             
            WaveVelC0Hxi (I,J)   = CosWaveDir(I)*Omega*tmpComplex* WaveElevxiGtr0 * COSHNumOvrSINHDen ( WaveNmbr, InitInp%WtrDpth, 0.0_ReKi )
            WaveVelC0Hyi (I,J)   = SinWaveDir(I)*Omega*tmpComplex* WaveElevxiGtr0 * COSHNumOvrSINHDen ( WaveNmbr, InitInp%WtrDpth, 0.0_ReKi )
             
            WaveVelC0V (I,J)     =           ImagOmega*tmpComplex* WaveElevxiGtr0 * SINHNumOvrSINHDen ( WaveNmbr, InitInp%WtrDpth, 0.0_ReKi )
