...
!    
!**********************************************************************************************************************************
! File last committed: $Date: 2016-06-30 12:40:32 -0600 (Thu, 30 Jun 2016) $
! (File) Revision #: $Rev: 678 $
! URL: $HeadURL: https://windsvn2.nrel.gov/HydroDyn/branches/WaveStretching/Source/Morison.f90 $
!**********************************************************************************************************************************
MODULE Morison
   use fruit
  ...
  ! USE HydroDyn_Output_Types
   ...
   TYPE(ProgDesc), PARAMETER            :: Morison_ProgDesc = ProgDesc( 'Morison', 'v1.01.00', '23-Dec-2015' )
      ...
      ! ..... Public Subroutines ...................................................................................................
   PUBLIC:: Test_BreakingWavesInit1
   PUBLIC:: Test_BreakingWavesInit2
   PUBLIC:: Test_BreakingWavesInit3
...
subroutine Test_BreakingWavesInit2()


   real(ReKi)                 :: alpha           !< Inclination angle of seabed in direction of wave velocity
   real(ReKi)                 :: lambda          !< Curling factor, set to 0.5
   real(ReKi)                 :: waveDir         !< Wave propagation direction [degrees, 0 = +X, 90 = +Y]]
   real(ReKi)                 :: Gravity
   real(SiKi)                 :: WtrDpth
   real(SiKi)                 :: WtrDens
   real(ReKi)                 :: R               !< Outer radius of cylinder
   real(ReKi)                 :: k(3)            !< vector along the cylinder's axial direction
   integer                    :: NStepWave  
   real(SiKi)                 :: waveDT
   real(SiKi), allocatable                 :: WaveElev(:)
   real(SiKi)                 :: WaveKinzi
   real(SiKi), allocatable                 :: WaveTime(:)
   real(SiKi), allocatable                 :: WaveVel(:,:)  
   real(ReKi), allocatable    :: D_F_BW(:,:)
   integer(IntKi)             :: ErrStat             !< Error status of the operation
   character(1024)            :: ErrMsg      !< Error message if ErrStat /= ErrID_None
   integer(IntKi)             :: I
   REAL(dBKi)                 :: simDT, simTime
   INTEGER                    :: NStepSim 
   real(ReKi) :: a,b,limit, periodSeconds, H, amp
   ...
   waveDT = 0.25_SiKi
   NStepWave = simTime/ waveDT
   NStepSim = simTime / simDT
  
   ! allocate working arrays
   
   ALLOCATE ( WaveElev(0:NStepWave), STAT = ErrStat )
   ALLOCATE ( WaveTime(0:NStepWave), STAT = ErrStat )
   ALLOCATE ( WaveVel(0:NStepWave,3), STAT = ErrStat )
   
   WaveKinzi = 0.0_SiKi
   do I=0,NStepWave
      WaveTime(I) = waveDT*I
      WaveElev(I) = 0.0_SiKi
      WaveVel(I,1) = 0.0_SiKi
      WaveVel(I,2) = 0.0_SiKi
      WaveVel(I,3) = 0.0_SiKi
   end do

   
   call BreakingWaves_Init( simDT, NStepSim, alpha, lambda, waveDir, Gravity, WtrDpth, WtrDens, R, k, NStepWave,  &
                               waveDT, WaveElev, WaveKinzi, WaveTime ,  &
                               WaveVel , D_F_BW, ErrStat  , ErrMsg )
   do I=0,NStepSim
      call assert_equals (0.0_ReKi, D_F_BW(I,1))
      call assert_equals (0.0_ReKi, D_F_BW(I,2))
      call assert_equals (0.0_ReKi, D_F_BW(I,3))
   end do
   

end subroutine Test_BreakingWavesInit2
subroutine Test_BreakingWavesInit3()
   ...
   a = 44*(1-exp(-19*tan(alpha)))
   b = 1.6/(1+exp(-19*tan(alpha)))
   periodSeconds = 300.0 ! seconds
   limit = b/(1/WtrDpth+a/(Gravity*periodSeconds**2))
   ...
   do I=0,NStepWave
      WaveTime(I) = waveDT*I
      WaveElev(I) = amp*sin(2*pi*WaveTime(I)/periodSeconds)
      WaveVel(I,1) = 2.3_SiKi
      WaveVel(I,2) = 0.0_SiKi
      WaveVel(I,3) = 0.0_SiKi
   ...
   deallocate(WaveTime)
   deallocate(WaveVel)
end subroutine Test_BreakingWavesInit3
subroutine Test_BreakingWavesInit1()
   ...
   deallocate(WaveTime)
   deallocate(WaveVel)
end subroutine Test_BreakingWavesInit1
      ...
      IF (EqualRealNos(sign(1.0_ReKi,WaveElev(IMOD)) + sign(1.0_ReKi,WaveElev(IMOD-1)),0.0_ReKi)) THEN
         ...
         IF ( sign(1.0_ReKi,WaveElev(IMOD)) > 0.0_ReKi ) THEN
            ...
            IF ( countTrough > 1 ) THEN 
               ...
               WaveNmbr   = WaveNumber ( 2.0*pi/period, Gravity, WtrDpth )
...
SUBROUTINE CreateDistributedMesh( simDT, NStepSim, densWater, gravity, SeabedAngle, MSL2SWL, wtrDpth, waveDir, WaveBreak, NStepWave, WaveTime, WaveElev, WaveVel, WaveAcc, WaveDynP, numNodes, nodes, nodeInWater, numElements, elements, &
  ...
  ! REAL(ReKi),ALLOCATABLE     ::  F_DP(:,:)
   ...
   !if (numBreakerNodes > 0 ) then
   !   ALLOCATE ( crossings( 0:NStepWave ), STAT = ErrStat )
   !   IF ( ErrStat /= ErrID_None ) THEN
   !      ErrMsg  = ' Error allocating space for the crossings breaking waves array.'
   !      ErrStat = ErrID_Fatal
   !      RETURN
   !   END IF
   !   ALLOCATE ( peaks( 0:NStepWave ), STAT = ErrStat )
   !   IF ( ErrStat /= ErrID_None ) THEN
   !      ErrMsg  = ' Error allocating space for the peaks breaking waves array.'
   !      ErrStat = ErrID_Fatal
   !      RETURN
   !   END IF
   !   ALLOCATE ( troughs( 0:NStepWave ), STAT = ErrStat )
   !   IF ( ErrStat /= ErrID_None ) THEN
   !      ErrMsg  = ' Error allocating space for the peaks troughs waves array.'
   !      ErrStat = ErrID_Fatal
   !      RETURN
   !   END IF
   !end if
   ...
   DO count=1,numDistribMarkers
     ...
     IF ( (WaveBreak ) .AND. ( nodes(J)%JointPos(3) > MSL2SWL ) ) THEN
         ...
         call BreakingWaves_Init(  simDT, NStepSim, SeabedAngle, 0.5, waveDir, gravity, wtrDpth, densWater,  nodes(J)%R, nodes(J)%R_LToG(:,3), NStepWave,  &
                               waveDT, WaveElev(:,J), nodes(J)%JointPos(3), WaveTime ,  &
