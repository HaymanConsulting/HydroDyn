...
! =========  HydroDyn_ParameterType  =======
  TYPE, PUBLIC :: HydroDyn_ParameterType
    ...
    INTEGER(IntKi)  :: NWaveElevOut      ! Number of wave elevation outputs [-]
    REAL(SiKi) , DIMENSION(:,:), ALLOCATABLE  :: WaveElevOut      ! Total wave elevation [-]
    REAL(SiKi) , DIMENSION(:,:), ALLOCATABLE  :: WaveElevOut1      ! First order wave elevation [-]
    REAL(ReKi) , DIMENSION(1:6)  :: AddF0      ! Additional pre-load forces and moments (N,N,N,N-m,N-m,N-m) [-]
    REAL(ReKi) , DIMENSION(1:6,1:6)  :: AddCLin      ! Additional stiffness matrix [-]
    REAL(ReKi) , DIMENSION(1:6,1:6)  :: AddBLin      ! Additional linear damping matrix [-]
    REAL(ReKi) , DIMENSION(1:6,1:6)  :: AddBQuad      ! Additional quadratic damping (drag) matrix [-]
...
IF (ALLOCATED(SrcParamData%WaveElevOut)) THEN
  i1_l = LBOUND(SrcParamData%WaveElevOut,1)
  i1_u = UBOUND(SrcParamData%WaveElevOut,1)
  i2_l = LBOUND(SrcParamData%WaveElevOut,2)
  i2_u = UBOUND(SrcParamData%WaveElevOut,2)
  IF (.NOT. ALLOCATED(DstParamData%WaveElevOut)) THEN 
    ALLOCATE(DstParamData%WaveElevOut(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%WaveElevOut.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%WaveElevOut = SrcParamData%WaveElevOut
ENDIF
IF (ALLOCATED(SrcParamData%WaveElevOut1)) THEN
  i1_l = LBOUND(SrcParamData%WaveElevOut1,1)
  i1_u = UBOUND(SrcParamData%WaveElevOut1,1)
  i2_l = LBOUND(SrcParamData%WaveElevOut1,2)
  i2_u = UBOUND(SrcParamData%WaveElevOut1,2)
  IF (.NOT. ALLOCATED(DstParamData%WaveElevOut1)) THEN 
    ALLOCATE(DstParamData%WaveElevOut1(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%WaveElevOut1.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%WaveElevOut1 = SrcParamData%WaveElevOut1
ENDIF
...
ENDIF
IF (ALLOCATED(ParamData%WaveElevOut)) THEN
  DEALLOCATE(ParamData%WaveElevOut)
ENDIF
IF (ALLOCATED(ParamData%WaveElevOut1)) THEN
  DEALLOCATE(ParamData%WaveElevOut1)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! WaveElevOut1 allocated yes/no
  IF ( ALLOCATED(InData%WaveElevOut1) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! WaveElevOut1 upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%WaveElevOut1)  ! WaveElevOut1
  END IF
      Re_BufSz   = Re_BufSz   + SIZE(InData%AddF0)  ! AddF0
      Re_BufSz   = Re_BufSz   + SIZE(InData%AddCLin)  ! AddCLin
      Re_BufSz   = Re_BufSz   + SIZE(InData%AddBLin)  ! AddBLin
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%WaveElevOut1) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevOut1,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevOut1,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevOut1,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevOut1,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%WaveElevOut1)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%WaveElevOut1))-1 ) = PACK(InData%WaveElevOut1,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%WaveElevOut1)
  END IF
      ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%AddF0))-1 ) = PACK(InData%AddF0,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%AddF0)
      ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%AddCLin))-1 ) = PACK(InData%AddCLin,.TRUE.)
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask2)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! WaveElevOut1 not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%WaveElevOut1)) DEALLOCATE(OutData%WaveElevOut1)
    ALLOCATE(OutData%WaveElevOut1(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%WaveElevOut1.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%WaveElevOut1)>0) OutData%WaveElevOut1 = REAL( UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%WaveElevOut1))-1 ), mask2, 0.0_ReKi ), SiKi)
      Re_Xferred   = Re_Xferred   + SIZE(OutData%WaveElevOut1)
    DEALLOCATE(mask2)
  END IF
    i1_l = LBOUND(OutData%AddF0,1)
    i1_u = UBOUND(OutData%AddF0,1)
