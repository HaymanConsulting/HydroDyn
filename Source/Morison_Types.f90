...
! =========  Morison_InitInputType  =======
  TYPE, PUBLIC :: Morison_InitInputType
    INTEGER(IntKi)  :: NStepSim      !  [-]
    REAL(ReKi)  :: Gravity      !  [-]
    REAL(ReKi)  :: WtrDens      !  [-]
    REAL(ReKi)  :: WtrDpth      !  [-]
    REAL(ReKi)  :: WaveDir      ! Needed for breaking waves.  The mean wave direction in degrees. [0 = +X, 90 = +Y] [-]
    LOGICAL  :: WaveBreak      ! Flag for including a breaking wave model and resulting impact loads. [unused when WaveStMod=0] -- set by HydroDyn_Input [-]
    REAL(ReKi)  :: SeabedAngle      ! Inclination angle of seabed in direction of wave velocity (degrees) [positive if rising in direction of waves] [-]
    REAL(ReKi)  :: MSL2SWL      !  [-]
...
! =========  Morison_MiscVarType  =======
  TYPE, PUBLIC :: Morison_MiscVarType
    ...
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: D_F_BW      ! Distributed breaking waves loads [-]
...
! =========  Morison_ParameterType  =======
  TYPE, PUBLIC :: Morison_ParameterType
    ...
    INTEGER(IntKi) , DIMENSION(:), ALLOCATABLE  :: distribToBreakingIndx      !  [-]
...
! 
   ErrStat = ErrID_None
   ErrMsg  = ""
    DstInitInputData%NStepSim = SrcInitInputData%NStepSim
    DstInitInputData%Gravity = SrcInitInputData%Gravity
    DstInitInputData%WtrDens = SrcInitInputData%WtrDens
    DstInitInputData%WtrDpth = SrcInitInputData%WtrDpth
    DstInitInputData%WaveDir = SrcInitInputData%WaveDir
    DstInitInputData%WaveBreak = SrcInitInputData%WaveBreak
    DstInitInputData%SeabedAngle = SrcInitInputData%SeabedAngle
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  Db_BufSz  = 0
  Int_BufSz  = 0
      Int_BufSz  = Int_BufSz  + 1  ! NStepSim
      Re_BufSz   = Re_BufSz   + 1  ! Gravity
      Re_BufSz   = Re_BufSz   + 1  ! WtrDens
      Re_BufSz   = Re_BufSz   + 1  ! WtrDpth
      Re_BufSz   = Re_BufSz   + 1  ! WaveDir
      Int_BufSz  = Int_BufSz  + 1  ! WaveBreak
      Re_BufSz   = Re_BufSz   + 1  ! SeabedAngle
  ...
  Int_Xferred = 1
      ...
      IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = InData%NStepSim
      Int_Xferred   = Int_Xferred   + 1
      ...
      Re_Xferred   = Re_Xferred   + 1
      IntKiBuf ( Int_Xferred:Int_Xferred+1-1 ) = TRANSFER( InData%WaveBreak , IntKiBuf(1), 1)
      Int_Xferred   = Int_Xferred   + 1
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%SeabedAngle
      Re_Xferred   = Re_Xferred   + 1
 ...
 ! buffers to store meshes, if any
  ...
  Db_Xferred  = 1
  Int_Xferred  = 1
      OutData%NStepSim = IntKiBuf( Int_Xferred ) 
      Int_Xferred   = Int_Xferred + 1
      ...
      Re_Xferred   = Re_Xferred + 1
      OutData%WaveBreak = TRANSFER( IntKiBuf( Int_Xferred ), mask0 )
      Int_Xferred   = Int_Xferred + 1
      OutData%SeabedAngle = ReKiBuf( Re_Xferred )
      Re_Xferred   = Re_Xferred + 1
...
ENDIF
IF (ALLOCATED(SrcMiscData%D_F_BW)) THEN
  i1_l = LBOUND(SrcMiscData%D_F_BW,1)
  i1_u = UBOUND(SrcMiscData%D_F_BW,1)
  i2_l = LBOUND(SrcMiscData%D_F_BW,2)
  i2_u = UBOUND(SrcMiscData%D_F_BW,2)
  IF (.NOT. ALLOCATED(DstMiscData%D_F_BW)) THEN 
    ALLOCATE(DstMiscData%D_F_BW(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%D_F_BW.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%D_F_BW = SrcMiscData%D_F_BW
ENDIF
...
ENDIF
IF (ALLOCATED(MiscData%D_F_BW)) THEN
  DEALLOCATE(MiscData%D_F_BW)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! D_F_BW allocated yes/no
  IF ( ALLOCATED(InData%D_F_BW) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! D_F_BW upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%D_F_BW)  ! D_F_BW
  END IF
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%D_F_BW) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%D_F_BW,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%D_F_BW,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%D_F_BW,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%D_F_BW,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%D_F_BW)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%D_F_BW))-1 ) = PACK(InData%D_F_BW,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%D_F_BW)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask2)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! D_F_BW not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%D_F_BW)) DEALLOCATE(OutData%D_F_BW)
    ALLOCATE(OutData%D_F_BW(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%D_F_BW.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%D_F_BW)>0) OutData%D_F_BW = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%D_F_BW))-1 ), mask2, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%D_F_BW)
    DEALLOCATE(mask2)
  END IF
...
ENDIF
IF (ALLOCATED(SrcParamData%distribToBreakingIndx)) THEN
  i1_l = LBOUND(SrcParamData%distribToBreakingIndx,1)
  i1_u = UBOUND(SrcParamData%distribToBreakingIndx,1)
  IF (.NOT. ALLOCATED(DstParamData%distribToBreakingIndx)) THEN 
    ALLOCATE(DstParamData%distribToBreakingIndx(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%distribToBreakingIndx.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%distribToBreakingIndx = SrcParamData%distribToBreakingIndx
ENDIF
...
ENDIF
IF (ALLOCATED(SrcParamData%D_F_BW)) THEN
  i1_l = LBOUND(SrcParamData%D_F_BW,1)
  i1_u = UBOUND(SrcParamData%D_F_BW,1)
  i2_l = LBOUND(SrcParamData%D_F_BW,2)
  i2_u = UBOUND(SrcParamData%D_F_BW,2)
  i3_l = LBOUND(SrcParamData%D_F_BW,3)
  i3_u = UBOUND(SrcParamData%D_F_BW,3)
  IF (.NOT. ALLOCATED(DstParamData%D_F_BW)) THEN 
    ALLOCATE(DstParamData%D_F_BW(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%D_F_BW.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%D_F_BW = SrcParamData%D_F_BW
ENDIF
...
ENDIF
IF (ALLOCATED(ParamData%distribToBreakingIndx)) THEN
  DEALLOCATE(ParamData%distribToBreakingIndx)
ENDIF
...
ENDIF
IF (ALLOCATED(ParamData%D_F_BW)) THEN
  DEALLOCATE(ParamData%D_F_BW)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! distribToBreakingIndx allocated yes/no
  IF ( ALLOCATED(InData%distribToBreakingIndx) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! distribToBreakingIndx upper/lower bounds for each dimension
      Int_BufSz  = Int_BufSz  + SIZE(InData%distribToBreakingIndx)  ! distribToBreakingIndx
  END IF
  ...
  Int_BufSz   = Int_BufSz   + 1     ! D_F_BW allocated yes/no
  IF ( ALLOCATED(InData%D_F_BW) ) THEN
    Int_BufSz   = Int_BufSz   + 2*3  ! D_F_BW upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%D_F_BW)  ! D_F_BW
  END IF
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%distribToBreakingIndx) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%distribToBreakingIndx,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%distribToBreakingIndx,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%distribToBreakingIndx)>0) IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(InData%distribToBreakingIndx))-1 ) = PACK(InData%distribToBreakingIndx,.TRUE.)
      Int_Xferred   = Int_Xferred   + SIZE(InData%distribToBreakingIndx)
  END IF
  ...
  ELSE
    ...
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%D_F_BW,2)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%D_F_BW,3)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%D_F_BW,3)
    Int_Xferred = Int_Xferred + 2
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! distribToBreakingIndx not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%distribToBreakingIndx)) DEALLOCATE(OutData%distribToBreakingIndx)
    ALLOCATE(OutData%distribToBreakingIndx(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%distribToBreakingIndx.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%distribToBreakingIndx)>0) OutData%distribToBreakingIndx = UNPACK( IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(OutData%distribToBreakingIndx))-1 ), mask1, 0_IntKi )
      Int_Xferred   = Int_Xferred   + SIZE(OutData%distribToBreakingIndx)
    DEALLOCATE(mask1)
  END IF
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%D_F_BW)) DEALLOCATE(OutData%D_F_BW)
    ALLOCATE(OutData%D_F_BW(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%D_F_BW.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ...
    END IF
    mask3 = .TRUE. 
      IF (SIZE(OutData%D_F_BW)>0) OutData%D_F_BW = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%D_F_BW))-1 ), mask3, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%D_F_BW)
    DEALLOCATE(mask3)
  END IF
