...
! =========  Waves_InitInputType  =======
  TYPE, PUBLIC :: Waves_InitInputType
    ...
    INTEGER(IntKi)  :: NWaveElevOut      ! Number of points where the incident wave elevations can be output [-]
...
! =========  Waves_InitOutputType  =======
  TYPE, PUBLIC :: Waves_InitOutputType
    ...
    REAL(SiKi) , DIMENSION(:,:), ALLOCATABLE  :: WaveElevOut      ! Instantaneous elevation time-series of incident waves at each of the NWaveElevOut points where the incident wave elevations can be output [(meters)]
    REAL(SiKi) , DIMENSION(:,:), ALLOCATABLE  :: WaveElevSt      ! Instantaneous elevation time-series of incident waves at each of the  points where wave kinematics are computed [(meters)]
...
! =========  Waves_ParameterType  =======
  TYPE, PUBLIC :: Waves_ParameterType
    REAL(SiKi)  :: DummyParameter      ! Remove this variable if you have parameter data [-]
...
! 
   ...
   ErrMsg  = ""
    ...
    DstInitInputData%NWaveElevOut = SrcInitInputData%NWaveElevOut
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  Int_BufSz  = 0
      ...
      Int_BufSz  = Int_BufSz  + 1  ! NWaveElevOut
  ...
  Int_Xferred = 1
      ...
      Re_Xferred   = Re_Xferred   + 1
      IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = InData%NWaveElevOut
      Int_Xferred   = Int_Xferred   + 1
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred  = 1
    ...
    DEALLOCATE(mask1)
      ...
      Re_Xferred   = Re_Xferred + 1
      OutData%NWaveElevOut = IntKiBuf( Int_Xferred ) 
      Int_Xferred   = Int_Xferred + 1
...
ENDIF
IF (ALLOCATED(SrcInitOutputData%WaveElevOut)) THEN
  i1_l = LBOUND(SrcInitOutputData%WaveElevOut,1)
  i1_u = UBOUND(SrcInitOutputData%WaveElevOut,1)
  i2_l = LBOUND(SrcInitOutputData%WaveElevOut,2)
  i2_u = UBOUND(SrcInitOutputData%WaveElevOut,2)
  IF (.NOT. ALLOCATED(DstInitOutputData%WaveElevOut)) THEN 
    ALLOCATE(DstInitOutputData%WaveElevOut(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitOutputData%WaveElevOut.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitOutputData%WaveElevOut = SrcInitOutputData%WaveElevOut
ENDIF
IF (ALLOCATED(SrcInitOutputData%WaveElevSt)) THEN
  i1_l = LBOUND(SrcInitOutputData%WaveElevSt,1)
  i1_u = UBOUND(SrcInitOutputData%WaveElevSt,1)
  i2_l = LBOUND(SrcInitOutputData%WaveElevSt,2)
  i2_u = UBOUND(SrcInitOutputData%WaveElevSt,2)
  IF (.NOT. ALLOCATED(DstInitOutputData%WaveElevSt)) THEN 
    ALLOCATE(DstInitOutputData%WaveElevSt(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitOutputData%WaveElevSt.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitOutputData%WaveElevSt = SrcInitOutputData%WaveElevSt
ENDIF
...
ENDIF
IF (ALLOCATED(InitOutputData%WaveElevOut)) THEN
  DEALLOCATE(InitOutputData%WaveElevOut)
ENDIF
IF (ALLOCATED(InitOutputData%WaveElevSt)) THEN
  DEALLOCATE(InitOutputData%WaveElevSt)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! WaveElevOut allocated yes/no
  IF ( ALLOCATED(InData%WaveElevOut) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! WaveElevOut upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%WaveElevOut)  ! WaveElevOut
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! WaveElevSt allocated yes/no
  IF ( ALLOCATED(InData%WaveElevSt) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! WaveElevSt upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%WaveElevSt)  ! WaveElevSt
  END IF
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%WaveElevOut) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevOut,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevOut,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevOut,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevOut,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%WaveElevOut)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%WaveElevOut))-1 ) = PACK(InData%WaveElevOut,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%WaveElevOut)
  END IF
  IF ( .NOT. ALLOCATED(InData%WaveElevSt) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevSt,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevSt,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%WaveElevSt,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%WaveElevSt,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%WaveElevSt)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%WaveElevSt))-1 ) = PACK(InData%WaveElevSt,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%WaveElevSt)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask3)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! WaveElevOut not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%WaveElevOut)) DEALLOCATE(OutData%WaveElevOut)
    ALLOCATE(OutData%WaveElevOut(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%WaveElevOut.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%WaveElevOut)>0) OutData%WaveElevOut = REAL( UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%WaveElevOut))-1 ), mask2, 0.0_ReKi ), SiKi)
      Re_Xferred   = Re_Xferred   + SIZE(OutData%WaveElevOut)
    DEALLOCATE(mask2)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! WaveElevSt not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%WaveElevSt)) DEALLOCATE(OutData%WaveElevSt)
    ALLOCATE(OutData%WaveElevSt(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%WaveElevSt.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%WaveElevSt)>0) OutData%WaveElevSt = REAL( UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%WaveElevSt))-1 ), mask2, 0.0_ReKi ), SiKi)
      Re_Xferred   = Re_Xferred   + SIZE(OutData%WaveElevSt)
    DEALLOCATE(mask2)
  END IF
...
! 
   ErrStat = ErrID_None
   ErrMsg  = ""
    DstParamData%DummyParameter = SrcParamData%DummyParameter
...
! 
 ...
 ! buffers to store subtypes, if any
  ...
  Db_BufSz  = 0
  Int_BufSz  = 0
      Re_BufSz   = Re_BufSz   + 1  ! DummyParameter
  IF ( Re_BufSz  .GT. 0 ) THEN 
     ALLOCATE( ReKiBuf(  Re_BufSz  ), STAT=ErrStat2 )
     IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating ReKiBuf.', ErrStat, ErrMsg,RoutineName)
  ...
  Int_Xferred = 1
      ...
      ReKiBuf ( Re_Xferred:Re_Xferred+(1)-1 ) = InData%DummyParameter
      Re_Xferred   = Re_Xferred   + 1
 ...
 ! buffers to store meshes, if any
  ...
  Db_Xferred  = 1
  Int_Xferred  = 1
      OutData%DummyParameter = REAL( ReKiBuf( Re_Xferred ), SiKi) 
      Re_Xferred   = Re_Xferred + 1
